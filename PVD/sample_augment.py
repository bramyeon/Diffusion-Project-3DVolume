...
from datasets.shapenet_data_pc import ShapeNetPointClouds
...
def get_dataset(dataroot, npoints,category,use_mask=False):
    tr_dataset = ShapeNetPointClouds(root_dir=dataroot,
        categories=[category], split='train',
        tr_sample_size=npoints,
        te_sample_size=npoints,
        use_mask = use_mask)
    te_dataset = ShapeNetPointClouds(root_dir=dataroot,
...
def generate(model, opt, savedir):
    ...
    with torch.no_grad():
        ...
        for iter in range(niter):
            ...
            samples = torch.cat(samples, dim=0)[:int(nmax)]

            samples = samples.cpu().numpy()
            np.save(os.path.join(savedir, f"samples{iter}.npy"), samples)
...
def main(opt):
    ...
    with torch.no_grad():
        ...
        if opt.generate:
            ...
            savedir = os.path.join(output_dir, 'syn')
            generate(model, opt, savedir)


def parse_args():

    parser = argparse.ArgumentParser()
    parser.add_argument('--dataroot', default='../data/preprocessed/pc')
    ...
    parser.add_argument('--nsamp', default=10000, help="number of samples to generate")
