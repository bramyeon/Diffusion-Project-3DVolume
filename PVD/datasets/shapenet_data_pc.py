...
class UniformPC(Dataset):
    def __init__(self, root_dir, categories, tr_sample_size=2048,
        ...
        self.cate_idx_lst = []
        self.all_points = []
        for cate_idx, category in enumerate(self.categories):
            # NOTE: [subd] here is synset id
            category_path = os.path.join(self.root_dir, self.split, category)
            if not os.path.isdir(category_path):
                print("Directory missing : %s" % category_path)
                continue

            for file in os.listdir(category_path):
                if file.endswith('.npy'):
                    obj_fname = os.path.join(category_path, file)
                    try:
                        point_cloud = np.load(obj_fname)
                        assert point_cloud.shape[0] == 2048
                        self.all_points.append(point_cloud[np.newaxis, ...])
                        self.cate_idx_lst.append(cate_idx)
                        self.all_cate_mids.append((category, file))
...
class ShapeNetPointClouds(UniformPC):
    def __init__(self, root_dir="data/ShapeNetCore.v2.PC15k",
                 categories=['airplane'], tr_sample_size=2048, te_sample_size=2048,
        ...
        if 'all' in categories:
            ...
            self.synset_ids = list(cate_to_synsetid.keys())
        else:
            # self.synset_ids = [cate_to_synsetid[c] for c in self.cates]
            self.synset_ids = [c for c in self.cates]
